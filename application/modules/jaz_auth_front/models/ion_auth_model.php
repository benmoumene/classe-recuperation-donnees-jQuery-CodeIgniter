...
class Ion_auth_model extends CI_Model
{
	...
	public function login($identity, $password, $remember=FALSE)
	{
		...
		//fetch the  
	    $query = $this->db->select($this->identity_column.', id, password, group_id, username')
			      ->where($this->identity_column, $identity)
			      ->where('active', 1)
			      ->where($this->ion_auth->_extra_where)
			      ->limit(1)
	    ...
	    if ($query->num_rows() == 1) //If record exist
	    {
				$password = $this->hash_password_db($identity, $password); //simply encrypt the password

				if ($result->password === $password) //if the password in the query equal to the encrypted password. Do the codes below
				{
						$this->update_last_login($result->id); //get the current date and update the date.
						
						//This will select name column in the groups table with a row only.
						//The groups table is about what level is the user is. example:  admin, members etc...
						$group_row = $this->db->select('name')
												->where('id', $result->group_id)
												->get($this->tables['groups'])
												->row();

						//The group_id is the foreign key from groups table.
						...
						//call the remember_user method to remember the data using the cookie.
