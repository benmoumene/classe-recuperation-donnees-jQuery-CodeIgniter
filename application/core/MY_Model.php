...
class MY_Model extends CI_Model {
     ...
     * @author Joost van Veen
     */
    public function save($data1, $data2, $id = FALSE) {
        if ($id == FALSE) {
            // This is an insert
            $this->db->set($data1)->insert($this->table_name1);
			$data2['userid'] = $this->db->insert_id();
			$this->db->set($data2)->insert($this->table_name2);
        }
        else {
            // This is an update
            $filter = $this->primaryFilter;
            $this->db->set($data1)->where($this->primary_key, $filter($id))->update($this->table_name1);
			$data2['userid'] = $this->db->insert_id();
			$this->db->set($data2)->where($this->primary_key, $filter($id))->update($this->table_name2);
        }
    ...
    public function is_admin($username, $password)
    {
    	...
    	//Check	if username and password exist in users table.
    	//select record where username = $username
		$q = $this->db->select('id, group_id, username, password')
				 ->from('users')
				 ->where('username', $username)
				 ->where('password', sha1($password))
				 ->where('group_id', 1)
